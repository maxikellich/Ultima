...
watch(
    () => layoutConfig.isNewThemeLoaded,
    (isLoaded) => {
        if (isLoaded) {
            refreshChart();
            layoutConfig.isNewThemeLoaded.value = false;
        }
    }
);
...
const changeMonthlyDataView = () => {
    if (monthlyRef.value.chart.config.options.scales.x.stacked) {
        monthlyRef.value.chart.config.options.scales.x.stacked = false;
        monthlyRef.value.chart.config.options.scales.y.stacked = false;
    } else {
        monthlyRef.value.chart.config.options.scales.x.stacked = true;
        monthlyRef.value.chart.config.options.scales.y.stacked = true;
    }

    monthlyRef.value.chart.update();
};
...
onBeforeUnmount(() => {
    if (storeInterval.value) {
        clearInterval(storeInterval.value);
    }
});
const isRTL = computed(() => {
    return layoutConfig.RTL;
});
</script>
        ...
        <div class="col-12 md:col-8">
            <div class="card h-full">
                <div class="flex align-items-center justify-content-between mb-3">
                    ...
                    <Button v-ripple label="Vertical/Stacked Data" class="p-button-text ml-auto" @click="changeMonthlyDataView"></Button>
                </div>

                <Chart ref="bar" type="bar" :data="chartMonthlyData" :options="chartMonthlyOptions" :height="400"></Chart>
            </div>
        </div>
        ...
        <div class="col-12 md:col-4">
            <div class="card h-full">
                <div class="flex align-items-center justify-content-between mb-2">
                    ...
                    <div>
                        <Button v-ripple icon="pi pi-ellipsis-h" class="p-button-rounded p-button-text p-button-plain" @click="toggleMenu"></Button>
                        <Menu ref="menu11" :popup="true" :model="items"></Menu>
                    </div>
                </div>
                <div class="border-bottom-1 surface-border text-sm text-color-secondary mb-2 flex align-items-center">
                    ...
                    <Button v-ripple label="Semi/Full Data" class="p-button-text ml-auto" @click="changeDoughnutDataView"></Button>
                </div>
                <Chart ref="doughnut" type="doughnut" :data="doughnutData" :options="doughnutOptions" :height="200"></Chart>
            ...
            <div class="card grid grid-nogutter block sm:flex">
                <div class="lg:col-3 md:col-6 sm:col-12 p-0">
                    <div class="flex flex-column p-4">
                        ...
                        <span class="text-2xl mt-2">
                            <i v-if="storeADiff !== 0" class="font-bold text-2xl pi pr-1" :class="{ 'pr-1': !isRTL, 'pl-1': isRTL, 'pi-arrow-up text-green-500': storeADiff > 0, 'pi-arrow-down text-pink-500': storeADiff < 0 }"></i>
                            ${{ storeATotalValue }}
                            <span v-if="storeADiff !== 0" class="font-medium text-base ml-1" :class="{ 'ml-1': !isRTL, 'mr-1': isRTL, 'text-green-500': storeADiff > 0, 'text-pink-500': storeADiff < 0 }">
                ...
                <div class="lg:col-3 md:col-6 sm:col-12 p-0">
                    <div class="flex flex-column p-4">
                        ...
                        <span class="text-2xl mt-2">
                            <i v-if="storeBDiff !== 0" class="fw-700 fs-large pi" :class="{ 'pr-1': !isRTL, 'pl-1': isRTL, 'pi-arrow-up text-green-500': storeADiff > 0, 'pi-arrow-down text-pink-500': storeADiff < 0 }"></i>
                            ${{ storeBTotalValue }}
                            <span v-if="storeBDiff !== 0" class="fw-500 fs-normal" :class="{ 'ml-1': !isRTL, 'mr-1': isRTL, 'text-green-500': storeBDiff > 0, 'text-pink-500': storeBDiff < 0 }"> {{ storeBDiff > 0 ? '+' : '' }}{{ storeBDiff }} </span>
                        </span>
                    </div>
                    <div class="px-4">
                ...
                <div class="lg:col-3 md:col-6 sm:col-12 p-0">
                    <div class="flex flex-column p-4">
                        ...
                        <span class="text-2xl mt-2">
                            <i v-if="storeCDiff !== 0" :class="{ 'pr-1': !isRTL, 'pl-1': isRTL, 'pi-arrow-up text-green-500': storeADiff > 0, 'pi-arrow-down text-pink-500': storeADiff < 0 }"></i>
                            ${{ storeCTotalValue }}
                            <span v-if="storeCDiff !== 0" class="fw-500 fs-normal" :class="{ 'ml-1': !isRTL, 'mr-1': isRTL, 'text-green-500': storeCDiff > 0, 'text-pink-500': storeCDiff < 0 }"> {{ storeCDiff > 0 ? '+' : '' }}{{ storeCDiff }} </span>
                        </span>
                    </div>
                    <div class="px-4">
                ...
                <div class="lg:col-3 md:col-6 sm:col-12 p-0">
                    <div class="flex flex-column p-4">
                        ...
                        <span class="text-2xl mt-2">
                            <i v-if="storeDDiff !== 0" class="fw-700 fs-large pi" :class="{ 'pr-1': !isRTL, 'pl-1': isRTL, 'pi-arrow-up text-green-500': storeADiff > 0, 'pi-arrow-down text-pink-500': storeADiff < 0 }"></i>
                            ${{ storeDTotalValue }}
                            <span v-if="storeDDiff !== 0" class="fw-500 fs-normal" :class="{ 'ml-1': !isRTL, 'mr-1': isRTL, 'text-green-500': storeDDiff > 0, 'text-pink-500': storeDDiff < 0 }"> {{ storeDDiff > 0 ? '+' : '' }}{{ storeDDiff }} </span>
                        </span>
                    </div>
                    <div class="px-4">
